...
import {IoFastFoodOutline, IoNutritionOutline, IoTimeSharp} from 'react-icons/io5';
        ...
        <div className="lg:hidden bg-white shadow-lg fixed bottom-0 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-500 border">
            {allTypes.map((item) => (
                <Link key={item.id} to={`/${item.id}`} className="w-1/4">
                    <div className={
                    type === item.id 
                        ? "flex flex-col text-center text-xl text-zomato-400" 
                        : "flex flex-col text-center text-xl"
                    }>
                        <div className={
                            type === item.id 
                            ? "w-full h-full border-t-2 p-3 border-zomato-400 flex-col flex items-center" 
            ...
            {allTypes.map((item) => (
                <Link key={item.id} to={`/${item.id}`}>
                    <div className={classnames(
                      "flex items-center gap-1 pb-2 transition duration-700 ease-in-out",
                      {
                        ...
                        <div className={classnames("w-16 h-16 bg-gray-100 p-4 rounded-full",
                        {
                          ...
                          <h3 className={
                            type === item.id ? "text-md ml-2 font-bold text-zomato-400" : "text-md ml-2 font-bold text-gray-500"
                          }>
                            {item.name}
                          </h3>
                    </div>
